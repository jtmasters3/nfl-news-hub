...
/**
 * artwork_requested -> artwork_created -> validating -> (artwork_ready | failed).
 * Lands on "artwork_ready", NOT "awaiting_approval" — Phase 2C requires a
 * valid caption before a story may ever reach awaiting_approval; see
 * captionEvents.js's applyCaptionCompleteEvent for the only place that
 * edge is taken. Artwork becoming durable here does not depend on
 * captions in any way, and is never regenerated by anything caption-related.
 * @param {object} state
...
export function applyCompleteEvent(state, payload, { reachable }) {
  ...
  if (passed) {
    const final = transition(step.state, story_id, "artwork_ready", {
