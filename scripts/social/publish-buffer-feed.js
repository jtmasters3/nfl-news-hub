#!/usr/bin/env node
// Production-capable one-story launcher for the approved Stage 5B Buffer
// Feed publishing orchestrator. A THIN DEPENDENCY-WIRING LAYER ONLY — every
// actual sequencing/durability decision (claim -> confirm -> checkpoint ->
// confirm -> Buffer -> map -> result -> confirm) lives in
// scripts/social-worker/lib/bufferFeedOrchestrator.js's
// executeBufferFeedPublish, never duplicated here. This file never contains
// embedded credentials; every secret comes from the environment only.
//
// Usage:
//   Dry-run / preflight (the default — always safe, NEVER mutates anything):
//     node scripts/social/publish-buffer-feed.js <story_id>
//
...
//     node scripts/social/publish-buffer-feed.js <story_id> --live --confirm-story <story_id>
//
...
//                                   configured channel (matches
//                                   cloudflare-worker/wrangler.toml's own
//                                   committed value)
//
...
// githubStateReader.js, optionally using GITHUB_API_TOKEN for a higher rate
// limit) and performs one read-only existence check against the public R2
// JPEG URL. It NEVER acquires a posting claim, NEVER dispatches a
// repository event, NEVER downloads/converts/uploads a JPEG, and NEVER
// calls Buffer. BUFFER_API_KEY is never read, needed, or referenced here —
// that secret remains Worker-only.
...
import { resolveApprovedFeedJpeg, checkExistingApprovedFeedJpeg } from "../social-worker/lib/resolveApprovedFeedJpeg.js";
import { executeBufferFeedPublish, validateBufferFeedPublishPreconditions } from "../social-worker/lib/bufferFeedOrchestrator.js";
...
// BUFFER_INSTAGRAM_CHANNEL_ID. The Worker independently enforces that the
// channelId a caller sends matches its own configured value — this default
// only saves a caller from having to know/pass it themselves.
 ...
 * exact --confirm-story match of the positional story_id. Missing either,
 * or a mismatched confirmation value, always means dry-run.
 */
 ...
 * every precondition, assembles the deterministic caption, and performs
 * ONE read-only existence check for the JPEG derivative — NEVER a PNG
 * download, NEVER a conversion, NEVER an upload, NEVER a posting claim,
 * NEVER a dispatch, NEVER a Buffer call. Safe to run at any time, any
 * number of times, with zero required Worker credentials.
...
export async function runPreflight({ storyId, fetchState, fetchImpl }) {
  ...
  let jpegReport;
  try {
    const check = await checkExistingApprovedFeedJpeg(record, { fetchImpl });
    jpegReport = !check.ok
...
/**
 * Delegates to the existing approved orchestrator exactly once. This
 * function itself implements NO retry, NO reordering, and NO duplicate
 * state logic — it only wires dependencies together. Every dependency is
 * overridable via `deps` (tests inject fakes here; real invocations get the
 * real wiring via the defaults below) — this function itself never calls
 * fetch directly.
...
export async function runLive({ storyId, deps = {} }) {
  ...
  const resolveJpeg = deps.resolveJpeg ?? ((record) => resolveApprovedFeedJpeg(record, { fetchImpl: fetch, uploadJpeg: bridge.uploadJpeg }));
  ...
  return executePublish({
    ...
    publishViaWorker: bridge.publishViaWorker,
 ...
 * would otherwise touch the network or real environment (fetchState,
 * fetchImpl, runLive/liveDeps) — real invocations get the real wiring via
 * the defaults below.
 */
export async function main(argv = process.argv.slice(2), deps = {}) {
  ...
  if (!args.storyId) {
    console.error("Usage: node scripts/social/publish-buffer-feed.js <story_id> [--live --confirm-story <story_id>]");
    process.exitCode = 1;
    return;
  }
  ...
  console.log(`=== LIVE MODE AUTHORIZED for story_id=${args.storyId} — delegating to executeBufferFeedPublish ===`);
